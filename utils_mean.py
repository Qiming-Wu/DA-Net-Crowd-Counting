...
def random_mask_batch_one_sample(batch, keep_per_image, mean_val=0.1307, reuse_noise=False):
    ...
    if reuse_noise:
        ...
        # out_c1 = out_c1.cuda().data.cpu().numpy()
        out_c1[:, idx] = flat[:, idx]
    else:
...
def avg_hard_forward(batch, net, num_samples, keep, gt_data):
    # expanded = batch.repeat_interleave(num_samples, 0)  # shape: batch*num_samples, etc
    masked = random_mask_batch_one_sample(batch, keep)
    masked = Variable(masked)
    soft = net(masked, gt_data)
    # votes = soft.max(1)[1]
...
def certify(batch, labels, net, alpha, keep, num_samples_select, num_samples_bound):
    guesses, _ = avg_hard_forward(batch, net, num_samples_select, keep, labels)

    bound_scores, _ = avg_hard_forward(batch, net, num_samples_bound, keep, labels)
    '''
    guesses.long()
    bound_scores.long()
    ...
    bound_selected_scores = lc_bound((bound_scores * num_samples_bound).cpu().numpy(), num_samples_bound, alpha)
    radii = population_radius_for_majority(bound_selected_scores, batch[0].nelement(), keep)
    # torch_tensor.nelement() 统计这个tensor里面元素的个数
