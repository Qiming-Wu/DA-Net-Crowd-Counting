...
# 替换成我的ablated_model
model_path = './saved_models/mcnn_Shanghai_A_certify_900.h5'
...
# 保存图片
if not os.path.exists('./defense_MCNN'):
    os.mkdir('./defense_MCNN')

if not os.path.exists('./defense_MCNN/density_map_adv'):
    os.mkdir('./defense_MCNN/density_map_adv')

if not os.path.exists('./defense_MCNN/images_adv_patch'):
    os.mkdir('./defense_MCNN/images_adv_patch')

if not os.path.exists('./defense_MCNN/images_adv_ablated'):
    os.mkdir('./defense_MCNN/images_adv_ablated')
...
# 存ablated   adv_patch攻击img
    ...
    plt.imsave('./defense_MCNN/images_adv_ablated/{}'.format(full_imgname), im_fi_save
               , format='png', cmap=plt.cm.jet)

    density_map = density_map.data.detach().cpu().numpy()
    adv_out = density_map[0][0]
    plt.imsave('./defense_MCNN/density_map_adv/{}'.format(full_imgname)
